...
class Chaplin:
    def __init__(self, use_ollama=True):
        ...
        # setup async ollama client (only if enabled)
        ...
        if self.use_ollama:
            try:
                ...
                print("💡 Running without text correction")
    ...
    async def correct_output_async(self, output, sequence_num):
        """Perform text correction using Ollama (if available)"""
        if not self.use_ollama or not self.ollama_client:
            # Fallback: just clean up the text and add punctuation
    ...
    def start_webcam(self):
        ...
        print("\n🎬 Webcam started!")
        print("📋 Instructions:")
        print("  - Press and hold ALT/OPTION to record")
        print("  - Press 'q' to quit")
        print("  - Speak clearly towards the camera")
        print()
        ...
        try:
                    ...
                    if ret:
                        ...
                        # check if not recording AND video is at least 2 seconds long
                            ...
                            if frame_count >= self.fps * 2:
                                ...
                                print(f"⏭️  Video too short ({frame_count} frames), skipping...")
            ...
            print(f"\n❌ Error during webcam operation: {e}")
        finally:
            print("\n🧹 Cleaning up...")
            # release everything
            cap.release()
